...
@section('title', 'Product FAQs | Zenotic Admin')
...
<div class="mx-auto max-w-6xl">
    <div class="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
            ...
            <h1 class="text-2xl font-bold text-gray-900">Product FAQs</h1>
            <p class="mt-1 text-sm text-gray-500">{{ $product->name }}</p>
        </div>
        @if($isEdit)
            <a href="{{ route('admin.products.faqs', $product) }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Add New</a>
        @endif
    </div>

    <form method="post" action="{{ $isEdit ? route('admin.products.faqs.update', [$product, $faq]) : route('admin.products.faqs.store', $product) }}" class="mb-8 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        @if($isEdit)
            @method('put')
        @endif
        ...
        <div class="flex items-center gap-5">
            ...
            @if($isEdit)
                <a href="{{ route('admin.products.faqs', $product) }}" class="text-sm font-medium text-gray-700 hover:text-gray-900">Cancel</a>
            @endif
        </div>
    </form>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-1">
                                    <a href="{{ route('admin.products.faqs.edit', [$product, $row]) }}" class="rounded-lg p-2 text-gray-500 hover:bg-blue-50 hover:text-blue-600" title="Edit">
                                        <i data-lucide="pencil" class="h-5 w-5"></i>
                                    </a>
                                    <form method="post" action="{{ route('admin.products.faqs.destroy', [$product, $row]) }}" onsubmit="return confirm('Delete this FAQ?')">
                                        @csrf
                                        @method('delete')
